...
from ...utils.helpers import convert_to_regex_list, is_hidden_name

# Legacy names for backward compatibility with settings files
MATERIAL_BLEND_MODE = MATERIAL_BLEND_MODES
MATERIAL_DEPTH_MODE = MATERIAL_DEPTH_MODES
...
class MaterialWriter(KN5Writer):
    def __init__(self, file, context, settings, warnings, texture_name_mapping=None):
        super().__init__(file)
        ...
        # Mapping of original texture name -> output name (for PNG->DDS conversions)
        self.texture_name_mapping = texture_name_mapping or {}
    ...
    def _fill_available_materials(self):
        ...
        for material in self.context.blend_data.materials:
            if material.users == 0:
            ...
            elif not is_hidden_name(material.name):
                ...
                material_properties = MaterialProperties(material, self.texture_name_mapping)
...
class MaterialProperties:
    def __init__(self, material, texture_name_mapping=None):
        self.name = material.name
        ac_mat = material.AC_Material
        self.shaderName = ac_mat.shader_name
        self.alphaBlendMode = int(ac_mat.alpha_blend_mode)
        self.alphaTested = ac_mat.alpha_tested
        self.depthMode = int(ac_mat.depth_mode)
    ...
    def copy_shader_properties(self, material):
        ac_mat = material.AC_Material
        properties = {}
        for shader_property in ac_mat.shader_properties:
        ...
        if not properties and ac_mat.shader_name == "ksPerPixel":
    ...
    def _generate_texture_mapping(self, material):
        ...
        for texture_node in texture_nodes:
            if texture_node.image and not is_hidden_name(texture_node.image.name):
                shader_input = texture_node.AC_Texture.shader_input_name
                original_name = texture_node.image.name
                # Use mapped name if PNG was converted to DDS
                texture_name = self.texture_name_mapping.get(original_name, original_name)
                mapping[shader_input] = texture_name
        return mapping
...
class MaterialSettings:
    def __init__(self, settings, warnings, material_settings_key):
        ...
        self.material_name_matches = convert_to_regex_list(material_settings_key)
    ...
    def apply_settings_to_material(self, material):
        ...
        for texture_mapping_name in texture_mapping_names:
            ...
            if not texture_name:
                msg = f"Ignoring texture mapping '{texture_mapping_name}' for material '{material.name}' without texture name"
