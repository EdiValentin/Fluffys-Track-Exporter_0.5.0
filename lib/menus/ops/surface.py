...
class AC_RefreshSurfaces(Operator):
    """Refresh surfaces with latest defaults while preserving custom surfaces"""
    bl_label = "Refresh Surfaces"
    bl_idname = "ac.refresh_surfaces"
    bl_options = {'REGISTER', 'UNDO'}

    def execute(self, context):
        settings: AC_Settings = context.scene.AC_Settings # type: ignore

        # Save current custom surfaces
        current_surfaces = {}
        for surface in settings.surfaces:
            if surface.custom:
                current_surfaces[surface.key] = surface.to_dict()
                current_surfaces[surface.key]["CUSTOM"] = True

        # Reload surfaces (this will load defaults + merge with saved customs)
        settings.load_surfaces(current_surfaces)

        self.report({'INFO'}, f"Refreshed surfaces: {len(settings.surfaces)} total")
        return {'FINISHED'}
...
class AC_AddSurface(Operator):
    ...
    def find_unique_value(self, key: str, field: str, settings: AC_Settings):
        ...
        # Strip any existing suffix (letters, hyphens, underscores)
        suffix_check = re.match(r"^(.*?)([_-][A-Z]+)?$", key)
        true_name = suffix_check.group(1).strip() if suffix_check else key.strip()

        # Generate letter-based suffixes: A, B, C, ... Z, AA, AB, ...
        import string
        for length in range(1, 4):  # Support up to ZZZ
            for combo in self._generate_letter_combinations(length):
                candidate = f"{true_name}_{combo}"
                if candidate not in keys:
                    return candidate

        # Fallback if somehow we run out of combinations
        return f"{true_name}_NEW"

    def _generate_letter_combinations(self, length: int):
        """Generate letter combinations: A-Z, AA-ZZ, AAA-ZZZ"""
        import string
