...
class AC_Preferences(AddonPreferences):
    ...
    ext_config_sync_mode: EnumProperty(
        name="ext_config.ini Sync Mode",
        description="How to handle external changes to ext_config.ini",
        items=[
            ('ASK', "Ask", "Ask what to do when external changes are detected"),
            ('OVERRIDE', "Always Override", "Always override file with addon data"),
            ('IMPORT', "Always Import", "Always import changes from file"),
        ],
        default='OVERRIDE',
    )
    ...
    def refresh_gizmos(self, context):
        if not context.scene.objects:
            return
        # Bounds check before accessing first object
    ...
    def draw(self, context):
        ...
        # ext_config sync settings
        box = layout.box()
        box.label(text="ext_config.ini Sync", icon="FILE_REFRESH")
        box.prop(self, "ext_config_sync_mode", text="")
